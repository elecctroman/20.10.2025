<section class="section-shell" aria-labelledby="checkout-title">
    ...
    <div class="checkout-grid">
        <form method="post" action="/checkout" class="checkout-card">
            ...
            <button type="submit">₺<?= number_format($totals['total'], 2) ?> tutarında ödemeyi tamamla</button>
        </form>
        <aside class="checkout-card" aria-labelledby="summary-title">
            ...
            <div class="summary-row"><span>Ara Toplam</span><span>₺<?= number_format($totals['subtotal'], 2) ?></span></div>
            <div class="summary-row"><span>İndirim</span><span>₺<?= number_format($totals['discount'], 2) ?></span></div>
            <div class="summary-row summary-total"><span>Ödenecek</span><span>₺<?= number_format($totals['total'], 2) ?></span></div>
            <?php if (!empty($coupon) && $totals['discount'] > 0): ?>
                <p class="input-note">Kupon: <?= sanitize($coupon['code']) ?> uygulanıyor.</p>
            <?php endif; ?>
