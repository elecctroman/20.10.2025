...
function app_log(string $level, string $message, array $context = []): void
{
    ...
    foreach ($context as $key => $value) {
        ...
        try {
            $normalized[$key] = json_decode(json_encode($value, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES), true, 512, JSON_THROW_ON_ERROR);
        } catch (Throwable $e) {
            $normalized[$key] = var_export($value, true);
        }
    }

    $contextString = $normalized ? json_encode($normalized, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) : '';
